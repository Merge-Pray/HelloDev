...
const GitHubAuthButton = ({ text = "Continue with GitHub", onSuccess, onError, className = "" }) => {
  ...
  const handleClick = () => {
    ...
    if (!clientId) {
      console.error('❌ VITE_GITHUB_CLIENT_ID not found in environment variables');
      if (onError) {
        onError('GitHub Client ID not configured. Please check environment variables.');
      }
      return;
    }
    ...
    console.log('🔍 GitHub OAuth Details:');
    ...
    if (!popup) {
      window.removeEventListener('message', handleMessage);
      if (onError) {
        onError('Popup was blocked. Please allow popups for GitHub authentication.');
      }
      return;
    }
    ...
    >
      {isLoading ? (
        ...
        <div className={styles.content}>
          <svg className={styles.githubIcon} width="18" height="18" viewBox="0 0 24 24">
